...
| **slowloris-keepalive** | Slow | Slowloris + Keep-alive |
...
```go
errType := errors.Classify(err)
if errors.IsRetryable(err) {
    // 재시도 로직
}
```

### 2.6. 메트릭 시스템 (Metrics System)
...
5. **에러 분류**: 실패 시 `errors.Classify()`로 에러 유형 분류.
